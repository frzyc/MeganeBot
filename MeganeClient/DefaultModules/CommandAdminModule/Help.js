      ...
      args: [
        {
          ...
          validate: (value, msg) => {
            ...
            if (match) {
              if (match[1])
                return { value: { commands: this.client.depot.findCommands(match[1].trim(), false, msg) } }
              if (match[2])
                return { value: { commands: this.client.depot.findCommands(match[2].trim(), false, msg) } }
              if (match[3])
                return { value: { modules: this.client.depot.findModules(match[3].trim(), false, msg) } }
              if (match[4])
                return { value: { modules: this.client.depot.findModules(match[4].trim(), false, msg) } }
            }
            const cmds = this.client.depot.findCommands(value, false, msg)
            const mods = this.client.depot.findModules(value, false, msg)
