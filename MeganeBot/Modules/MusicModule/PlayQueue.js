...
module.exports = class PlayQueue {
  ...
  constructor(client, guildID) {
    ...
    /**
     * A queue before items can be added to the playlist. 
     * This ensures that we can add a song every 3 seconds
     */
    this.queue = []
    this.queuing = false
  }
  async addToList(track) {
    if (this.list.length >= PlayQueue.MAX_NUM_SONGS_PER_PLAYLIST)
      return this.client.autoMessageFactory({
        destination: track.message,
        messageContent: `The Playlist size is at maximum: ${PlayQueue.MAX_NUM_SONGS_PER_PLAYLIST}`,
        deleteTime: 30 * 1000
      })
    track.trackId = this.getTrackId()//generates a unique trackID for each song, even if it has been requeued
    this.list.push(track)
    if (this.list.length === 1) this.updatePlayingMessage()//update the next playing part of playing message, since it will show up as the next song
    ...
    if (!this.queuing) {
      ...
      setTimeout(() => {
        ...
        if (this.queue.length >= 1)
          this.addToList(this.queue.shift())
      }, 3000)
    } else
  ...
  removeFromList(trackId) {
    ...
    if (ind < 0) return console.log(`removeFromList:${trackId} FAILED TO FIND VIDEO IN QUEUE`)
  ...
  queryYTDL(searchString, message) {
    YoutubeDL.exec(searchString, ["--quiet", //Activate quiet mode
      ...
      const errMsg = this.client.messageFactory({
        destination: message,
        reply: true,
      ...
      else {//will return an array[0] of JSONstrings
        const queryInfo = queryInfo.map((info) => {
          try {
        ...
        if (queryInfo.url) {//single track
          ...
          queryInfo.entries.forEach((entry, index) => {
            setTimeout(() => {
              ...
              this.queryYTDL(entry.url, message)
            }, index * 2000)
          })
        }
      }
