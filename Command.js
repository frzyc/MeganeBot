...
module.exports = class Command {
     ...
     * TODO numArguments 1 -> use the entire argument string as a single argument, for when no args, or parse several...
     * TODO remember a command can have no arguments deal with dat stuff ^^
     */
    /**
    ...
    //TODOBREAKING single usage now... 
    getUsage(ind) {
        if (!this.usage) return `This command does not have a usage description.`
        else {
            let usagetxt = `Usage of **${this.name}${this.aliases.length > 0 ? ", " + this.aliases.join(", ") : ""}**:`;
    ...
    hasPermissions(message) {
        if (this.clientPermissions &&
            ...
            if (missing.length > 0) {
                util.createMessage({ messageContent: `I don't have enough permissions to use this command. missing:\n${missing.map(p => permissions[p]).join(', and ')}`, deleteTime: 5 * 60 * 1000 });
                return false;
            }
        }
        if (this.userPermissions &&
            ...
            if (missing.length > 0) {
                util.createMessage({ messageContent: `You don't have enough permissions to use ${this.name}. missing:\n${missing.map(p => permissions[p]).join(', and ')}`, deleteTime: 60 * 1000 });
                return false;
            }
        }
    }
